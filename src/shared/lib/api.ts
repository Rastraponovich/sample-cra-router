...
const registration = async (user: TRegistrationCredential) => await bookingAPIInstance.post("/auth/registration", user)
...
const modifyUser = async ({ id, ...user }: TUser) => await bookingAPIInstance.patch(`/users/${id}`, { ...user })
